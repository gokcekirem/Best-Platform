        ...
        override fun call(): SignedTransaction {
            val signTransactionFlow = object : SignTransactionFlow(otherPartySession) {
                override fun checkTransaction(stx: SignedTransaction) = requireThat {
                    ...
                    "This must be an MarketTime Initiation transaction." using (output is MarketTimeState)
                    val marketT = output as MarketTimeState
                    "MarketTime with a value not equal to 0 is not possible." using (marketT.marketTime == 0)
                    //A MarketTime other than 0 should not be possible since this is the initiation flow
                }
            }
